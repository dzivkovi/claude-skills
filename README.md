...
Two kinds of skill live here. Most are **claude.ai capability skills**: a portable `.zip` you upload once to Claude's settings, after which Claude produces the styled output on its own, with no prompting or reminding. Five are **Claude Code workflow skills** that orchestrate your local machine: [read-along](./skills/read-along/) drives your repo, `gh` history, and a real browser to give you a guided tour of work an AI already did, [markdown-to-epub](./skills/markdown-to-epub/) turns Markdown into clickable, listenable EPUBs (via local pandoc) for your Kindle, [markdown-to-pdf](./skills/markdown-to-pdf/) prints Markdown to plain, internal-grade PDFs with clickable links, real tables, and locally rendered Mermaid diagrams, [markdown-to-book](./skills/markdown-to-book/) bundles a folder of markdown chapters into one book in three lockstep editions (an agent-readable .md with provenance frontmatter, an EPUB, a PDF) by orchestrating the two converters above, and [markdown-to-confluence](./skills/markdown-to-confluence/) drafts a Confluence Cloud page from the same Markdown, rendering Mermaid locally so diagrams arrive as pictures instead of as code. Different species, same library. The distinction is spelled out under [Skills in this repo](#skills-in-this-repo) so the workflow skills do not get read as just another document styler.
...
They fall into two groups: five Claude Code workflow skills that drive your machine, and a set of claude.ai capability skills you upload and forget.
...
**[markdown-to-pdf](./skills/markdown-to-pdf/)** prints Markdown as a plain, internal-grade PDF - the working-document sibling of the branded skills, and of markdown-to-epub above (EPUB is the reading format, this is the printing one). It exists because the two obvious conversion routes each fail half the job: browser print pipelines render beautiful tables and Mermaid diagrams but emit PDFs whose links are all dead text (measured: zero /URI annotations out of headless Chrome, and no flag fixes it), while lightweight PDF writers keep links alive but print tables as raw pipe characters. Its single reportlab engine does both halves: every hyperlink stays clickable (including timestamped video deep-links, even inside table cells), pipe tables become real grids with shaded headers, ```mermaid fences render locally via mermaid-cli (no network, no kroki dependency), and every page carries light stamps chosen by audience: Page N of M and the date always, plus - depending on `--stamp-preset` - the source filename in the footer when you are printing your own research (so a page that falls out of the stack still names the markdown it came from), or your contact line when the PDF is going to someone else. Positions stay fixed on purpose: the preset chooses what is stamped, never where. Deliberately no cover page, no TOC, no brand tokens: that restraint is what lets the output read as an internal working document rather than a consulting artifact. Its SKILL.md is self-sufficient.

**[markdown-to-book](./skills/markdown-to-book/)** bundles a folder of markdown chapters into one book published in three lockstep editions sharing one basename: a single-file `.md` agent edition, plus the `.epub` reading edition and `.pdf` print edition via the two sibling skills above. It exists because a multi-file knowledge folder has three readers with different needs - you on a Kindle, you at a printer, and increasingly an AI agent that wants the whole corpus as ONE file it can take as context. The agent edition opens with a small provenance frontmatter header (origin repo@commit, the exact source files in order, a one-line purpose), so a receiving agent knows what it holds, where it came from, and whether it might be stale - an agent-to-agent handoff contract kept deliberately smaller than any protocol. The bundler is one stdlib-only script (ordered concatenation, link hygiene for standalone reading, the provenance header); conversion stays owned by markdown-to-epub and markdown-to-pdf - this skill is the orchestrator whose real job is keeping the three editions from silently drifting apart. Its SKILL.md is self-sufficient.

**[markdown-to-confluence](./skills/markdown-to-confluence/)** drafts a Confluence Cloud page from a Markdown file, and is the wiki-page sibling of the print and e-reader converters above. It exists because of one specific defeat: paste a document containing ```mermaid fences into Confluence and every diagram arrives as a code block showing its own source. The reader gets the recipe instead of the picture. This skill renders those fences locally with mermaid-cli first, exactly as markdown-to-pdf does, so what lands on the page is a diagram.

The bigger surprise was what turned out to be unnecessary. The obvious build is a pandoc pipeline into Confluence's HTML, which then needs a long list of attributes stripped because Confluence's validator rejects what pandoc emits. That whole layer can be deleted: the Atlassian MCP accepts raw Markdown and converts headings, pipe tables, fenced code, lists, blockquotes, links, and images to native Confluence storage format on its own, correctly. So the skill sends Markdown, and the only preprocessing that earns its place is the diagrams plus lifting the leading H1 out, because the page title is a separate argument.

The one real tradeoff is what to do with a rendered diagram, and the skill makes it an explicit choice rather than a default you discover later. Inlined as a base64 data URI, a diagram needs no attachments and no API token, but the body grows by roughly 1.35x the image bytes and every one of those bytes crosses the tool call: a three-diagram architecture document measured 128 KB inline against 9 KB with placeholders, where each diagram becomes a one-line marker you drag the rendered PNG onto. The PNGs are written to disk either way, so the manual path is always available as a fallback. Two limits are worth knowing before you meet them: the Atlassian MCP has no attachment scope at all, which is why placeholders is a first-class mode rather than a workaround, and a draft cannot be flipped to published by updating it. This skill targets Confluence Cloud only. Self-hosted Data Center speaks a different API, so the working pattern there is to draft on Cloud and paste the body into the destination in edit mode, which carries text, tables, and images across in one go.

**It has a hard dependency the others do not: the Atlassian MCP server, connected and authenticated for your site.** There is no REST fallback, and an agent cannot complete the OAuth flow on your behalf, so authorize it in an interactive session first or the skill cannot do anything at all. mermaid-cli is needed only when the document actually contains diagrams. Its SKILL.md is self-sufficient.
...
claude-skills/
  skills/
    ...
    markdown-to-confluence/           # Skill: draft a Confluence Cloud page from Markdown (Claude Code)
      SKILL.md                          # MCP publish procedure, image-mode tradeoff, Cloud-only limits
      scripts/
        prepare_confluence_markdown.py  # Renders mermaid to PNG, lifts the H1 out as the page title
  releases/
