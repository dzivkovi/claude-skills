...
`markdown-to-book` calls into `markdown-to-epub` (make_cover.py, epub.css, pandoc recipe) and `markdown-to-pdf` (print_markdown.py flags). The dependency is on those skills' **script interfaces**, not their internals. Before releasing a change to either converter, `rg` the changed script/flag name across `skills/` for consumers; a breaking interface change is a MAJOR bump for the converter AND requires updating and re-releasing `markdown-to-book` in the same cycle. Internal changes (rendering quality, styling) need nothing. The same rule applies to any future skill that layers on another (branded-docx/branded-pptx layer on Anthropic's base skills the same way, but those live outside this repo, so the risk there is upstream drift, not our releases).
...
- **markdown-to-book** — Bundles an ordered collection of markdown files into one book in three lockstep editions sharing one basename: a single-file `.md` agent edition with provenance frontmatter (origin repo@commit, source list, one-line purpose - the context handoff for another AI agent), an `.epub` via markdown-to-epub, and a `.pdf` via markdown-to-pdf. One stdlib-only script (`scripts/bundle.py`) does the deterministic bundling; the sibling skills own conversion. Encodes the lockstep rule (a chapter edit invalidates all three editions) and Mermaid render validation. A Claude Code workflow skill like read-along; SKILL.md is self-sufficient. Currently version 0.1.0.
...
- **markdown-to-epub** — Converts Markdown (one file or a bundled book of many) into clickable, listenable EPUBs for Kindle via local pandoc, with an optional Pillow-drawn branded cover carrying a date stamp. A Claude Code workflow skill like read-along; SKILL.md is self-sufficient. The conversion itself is deliberately scriptless (native pandoc); the only code files are the pandoc preflight and the cover generator.
- **markdown-to-pdf** — Prints Markdown to a plain, internal-grade PDF: clickable links everywhere (including inside table cells and timestamped video deep-links), real tables, ```mermaid fences rendered locally via mermaid-cli, and light header/footer stamps chosen by an audience preset (`--stamp-preset research|client|minimal`): Page N of M and the date always, plus the source filename in the footer when printing your own research or your contact line when sending a deliverable. Positions are fixed by design - a preset picks what is stamped, never where, because Anthropic's own best-practices doc names "offering too many options" an anti-pattern and prescribes a default with an escape hatch. One reportlab engine in `scripts/print_markdown.py` (PEP 723 header, `uv run` resolves deps). Converged from two earlier converters after headless Chrome's print path was measured to emit zero /URI link annotations (issue #4). Deliberately no cover/TOC/brand tokens - that is branded-docx territory. A Claude Code workflow skill; `assets/sample.md` is the install smoke test.
