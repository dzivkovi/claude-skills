...
- **markdown-to-pdf** — Prints Markdown to a plain, internal-grade PDF: clickable links everywhere (including inside table cells and timestamped video deep-links), real tables, ```mermaid fences rendered locally via mermaid-cli, and light header/footer stamps (Page N of M, version + date, author contact line). One reportlab engine in `scripts/print_markdown.py` (PEP 723 header, `uv run` resolves deps). Converged from two earlier converters after headless Chrome's print path was measured to emit zero /URI link annotations (issue #4). Deliberately no cover/TOC/brand tokens - that is branded-docx territory. A Claude Code workflow skill; `assets/sample.md` is the install smoke test.
